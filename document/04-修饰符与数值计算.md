...
> ⚠ **两种列表「遍历」与「取大小」字段都不同**（详见 [01-词法 §4.3](01-词法、数据类型与值系统.md#43-变量列表List)）：
> - **作用域内临时 list**（`add_to_list` 攒起、随脚本链存活）→ 遍历 `list = X`，取大小 `list_size:X`
> - **对象附属的 variable list**（`add_to_variable_list` 持久保存于角色/对象）→ 遍历 `variable = X`，取大小用 `variable_list_size`（trigger 块），**不用 `list_size:X`**；script value 数值上下文里计数改为 `has_variable_list` 保护 + `every_in_list = { variable = X add = 1 }`
> 两者不可混用，否则读不到数据。

```paradox
# ── 作用域内临时 list（add_to_list → 遍历 list = / 大小 list_size:）──
...
list_size:X > 0                        # 临时 list 的数值大小

# ── 对象附属 variable list（add_to_variable_list → 遍历 variable = / 大小 variable_list_size）──
add_to_variable_list     = { name = X  target = scope:y }      # 添加成员
remove_list_variable     = { name = X  target = scope:y }      # 移除成员
clear_variable_list = X                                        # 清空整个列表
has_variable_list = X                                          # 列表是否存在
is_target_in_variable_list = { name = X  target = scope:y }    # 成员判断（trigger）
variable_list_size = { name = X  value < N }                   # 列表大小判断（trigger 块）
...
- **`min` / `max` 是钳制，不是"取值"** —— `value = { … min = 0 }` 把低于 0 的结果抬回 0；**误写 `max = 0` 会把正结果压成 0**（难排查：值恒为 0 而不报错）。要下限用 `min`，要上限用 `max`。
- **`order_by` 实时求值要求"被评估者是 `this`"** —— 在 story 等非候选人域里排序（例如按"君主对候选人的评分"）时，先 `save_scope_as` 把被评估方存进来，由它发起求值；跨域直接引用会读到空值。
- **交互 AI 块的 `base` 不接受 script value** —— 写 `base = <某个 _value>` 报 `Malformed token`；脚本值一律走 `modifier = { add = <某个 _value> }`。
- **好感没有"按 opinion modifier 读净额"的 API** —— 引擎只提供 `add_opinion` / `remove_opinion` 与"读角色总好感"；需要按修饰符看净额时，得自己另存影子数值。
