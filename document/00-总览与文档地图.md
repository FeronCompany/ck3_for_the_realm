...
本系列共 **18 篇**，按"基础 → 工具 → 调度 → 系统 → 参考"五层组织。
...
| **17** | [GUI 界面设计与 scripted_guis](17-GUI界面设计与scripted_guis.md) | GUI 三大文件域、控件/布局语法、数据绑定、状态机、`blockoverride`、scripted_guis 数据同步 | ~800 | 01·02·06 |
...
| `change_gold` | 全目录 0 命中（资金效果只有 `add_gold` / `transfer_gold`，曾在 councillor 事件误用报错） |
| `add_trait = X` 写在 trigger 块内 | trigger 只读，加特质只能用 `has_trait`；曾在 court trigger 误用 |
| `dna = { }` 写在 `create_character` 内 | `dna` 不是 create_character 的键，曾致 decision 事件解析错误 |

### 10.2b 引擎内置 / 特殊定义位置（无 scripted_triggers 文件，但合法）

| 名字 / 位置 | 实证 |
|---|---|
| `is_valid_agent_standard_trigger` | 原版 **所有 scheme 的 `valid_agent`** 都在用，但全 game 的 `scripted_triggers/` 目录 **0 定义文件** —— 它是引擎为 scheme 提供的**内建触发**，勿误判为拼写错误，也不要自己补定义 |
| `scripted_effect X = { }` 写在 **events 文件顶层** | 原版 `birth_events.txt` / `kurultai_task_events.txt` 等大量先例。`scripted_effect`/`scripted_trigger` 关键词定义**不限于** `common/scripted_effects/` 目录 |
| `save_scope_value_as = { name = X }` 的 `X` | 原版 981 个不同 name 均**无需**在 `script_values/` 预定义 —— 它是运行时动态作用域值，用 `scope:X` 读取，与 script value 是两套系统 |
...
| 7 | 数值优先内联；跨文件复用 / 复杂公式才抽 script value | [04](04-修饰符与数值计算.md) §3 |
