...
| 70 | 强权下沿：请愿落地时的初始天平见 04 §2.1（被逼立共治者一律 40）。**太子立储另设两档初值**：主动立储（指定继承人）= 10、被逼立储（请愿・逼宫）= 40（03 §2.2 / §6） |
...
选族（on_action ftr_court_struggle_main_pulse：求值域落到 story，random_list **按烈度分档**；
  ...
  ★ 条目级「可执行性过滤」（2026-09 新增）：档内**每行自带 trigger**，跑不通的族被引擎剔出抽签池、
    其余条目**权重重新归一**——避免「抽中一族却因选不出人而空转」，把概率还给可执行的族：
      · 互殴族条目 → `ftr_court_has_two_participants_trigger`（参与者 ≥2 才凑得出 actor + victim）
      · 文明族条目 → `ftr_court_realm_has_camp_backer_trigger`（朝中至少一名「已入营党羽」）
...
4. **背书君主 / 共治者的 AI 倾向**（**专用权重**，不套用候选的通用权重——通用权重含双向好感 ×0.5、黑马、拥挤惩罚、立场契合，对在位目标不适用）：
   ...
   - **支持对象健康（2026-09 新增，三项通用）**：a–e / g / h 三处权重各含线性项 `8 × 对象 health − 24`（以官方档 fine=3 为中性：dying≈−24 / poor≈−16 / fine≈0 / good≈+16 / excellent≈+32）——投资行将就木者不划算，目标越体弱越不值得押注。与「决策层健康驱动」互补：前者管**是否下场**，本项管**押谁**（这正是「坚定站队不健康君主」的纠偏点）。
5. **0200.f「静观其变」的加码与关键人物抑制（2026-09）**：观望 `base` 由 35 抬到 **45**——不站队成为常见选择；但对**关键人物**额外 **−35**（`ftr_court_wait_key_figure_ai_modifier`）——举足轻重者不得轻易置身事外，须被迫下场表态。「关键人物」判定（`ftr_court_wait_key_figure_trigger`，六类任一成立）：① 军力 ≥ 最高领主 `ftr_court_wait_key_vassal_strength_ratio_value`=**10%** 的封臣；② **内阁成员**；③ **君主宫廷职位持有者**（`any_court_position_employer`，廷臣或封臣均可，比 `has_any_court_position` 更精确）；④ **君主配偶 / 后宫**（`is_spouse_of` / `is_consort_of`）；⑤ **君主继承人**（主头衔顺位第一 `is_heir_of` 或已指定储君 `ftr_court_is_designated_heir_of_realm_trigger`）；⑥ **强力封臣**（`is_powerful_vassal`）。**性格抑制（2026-09）**：上述强制表态对**不贪恋权位 / 胆小谨慎**者**仅下调压迫强度**（**不作完全豁免**），并分两档——**强档 +25（净 −10）**：`content` 安于现状 / `craven` 怯懦 / `humble` 谦卑；**常规档 +18（净 −17）**：`paranoid` 多疑 / `patient` 持重 / `shy` 怕生 / `lazy` 疏懒 / `trusting` 轻信（两档互斥，强档优先）。即「位高权重但本性不爱争 / 不敢争」者仍会被推着表态，只是压力明显较轻。
6. **仅约束 AI**：以上摩擦与倾向只进 AI 权重，不改交互 / 选项的可见性与触发条件，玩家仍可自由表态与改投。
...
2. **按对象属性分流类型**：授权对象属**合法继承人之一**（`ftr_court_is_legitimate_heir_of_trigger`：主头衔继承顺位上的非私生 / 未被剥夺者）→ 复用 03 的 `ftr_court_establish_champion_coregency_effect` 按政体分流——**罗马 / 欢呼法 → 共治皇帝 `co_emperorship`｜其他行政制 → 太子 `ftr_diarchy_crown_prince`｜其他政体 → 共治君主 `co_monarchy`｜缺省 → 固化摄政**（**被逼立共治者初始天平一律 40**，内联于统一关节，04 §2.1）；授权对象**不属**继承人线 → 由统一关节落入官职 / 摄政型分支（摄政 / 维齐尔 / 草原摄政）；
3. **回退摄政**：授权对象不属继承人线时，由统一关节按其政体落入 `regency` / `vizierate` / `nomad_regency`，天平同为 **40**（04 §2.1）。**前置**：授权对象须满足 `ftr_court_champion_eligible_diarch_trigger`（成年能力者）——**未成年授权对象不即时共治**，待其成年由 03 的 `ftr_diarchy_promote_adult_heir_effect` 补立（§2.2 1.3）；
...
**表态支持的 AI**：`ftr_court_declare_backing_decision.ai_will_do` 按改投摩擦核算（见上）。**君主健康驱动**——AI 押注意愿以最高统治者（actor 的 `top_liege`）健康值做**线性加成**：`加成 = 38.5 − 7 × health`（官方健康档 dying=0 / poor=1 / fine=3 / medium=4 / good=5 / excellent=7 → 约 +38.5 / +31.5 / +17.5 / +10.5 / +3.5 / −10.5，系数内联在决议 `ai_will_do`），健康越低加成越高。**2026-09 平衡：整体权重较初版（55 / 10）下调 30%**——避免老君主一次性把全朝拉下场；「支持对象本身的健康」另在 0200 选项权重中承担（见 §7.1-4-健康项）。被支持者一侧经事件 0210 裁决（`ftr_court_struggle.0210` 的 `ai_chance`）。
...
**「入营」口径（2026-09 统一）**：凡判定「是否党羽 / 是否已站队」处，一律走 `ftr_court_has_backing_trigger`（= 已有 `ftr_court_backer_of` ∧ 背书对象**仍是权贵**）；而权贵 = **最高君主 / 共治者 / 角逐竞争者**三类，故**拥护君主、拥护共治者的人同样是党羽**，其阵营同样参与 `same_camp` 对照。旧写法「`has_variable = ftr_court_backer_of` ∧ 背书对象 ≠ 君主」把君主 / 共治者两营整块排除——只剩 1 名竞争者时全场同属一营，双人机制（M2/M3/M4/M5）永远凑不出「两个不同阵营」，已在 `charge` / `sow_discord` 与 §12 文明族选角中废止。

| 交互 | 情形 | 权重 |
|---|---|---|
| `ftr_charge_interaction` | 目标属**其他党羽**（双方**均已入营**且非同营）→ 优先清除政敌 | +60 |
...
| `ftr_sow_discord_interaction` | 目标属**其他党羽**（双方**均已入营**）→ 打击敌对阵营 | +60 |
...
- **退出判定**（逐名）：基础 10%，顺从性格加分——懦弱 +20 / 谦卑・忠信（loyal）+15 / 轻信・公正・安于现状 +10，对君主好感 ≥40 再 +15；反向——傲慢・野心勃勃 −15 / 勇敢 −10，好感 <0 再 −15；求和后**整体 ×1.1（2026-09 平衡）**，夹在 0–66（`ftr_court_discipline_exit_chance_value`）。屈服者解除背书（清 `ftr_court_backer_of` 与拥立账）并从党首名单除名，有屈服才 interface_message 通报君主。实现口径：先打 2 天临时 flag 标记屈服者，再逐个排干名单（清空写法与遍历安全见 [03](../03-触发器与效果.md) §13.2）。
- **硬顶不退者的代价（2026-09 新增）**：名单排干后剩下的（未屈服）党羽——① 与最高领主关系进一步恶化（`ftr_court_disciplined_defiance_opinion` **−70 / 15 年**，重于被敲打权贵本人的 −50）；② 按 `ftr_court_discipline_defy_rival_chance_value`（**39 ≈ 默认结怨 35 × 1.1**）推进「结为仇敌」。
- **代价**：被敲打的权贵记恨（`ftr_court_disciplined_opinion` −50/10 年，可叠加）。
...
4. **调度侧「无目标不掷骰」（2026-09 修正）**：互殴族先抽 actor、再抽 victim（victim 须排除 actor 与 story_owner）。参与者不足 2 人时 victim 抽不到，`ftr_court_strife_slander_effect` **直接短路、不再掷手段概率**——否则只会派发「无目标」的空转事件（选项体全被 `scope:ftr_court_victim ?= {}` 跳过，`0430.a` 的 `start_scheme` 还会裸用未设 scope 报错），并白吃主时钟分给互殴族的权重；主时钟的互殴族条目另挂 `ftr_court_has_two_participants_trigger`（§4.3）做双重兜底。
...
- **续写约定**：继承 root / scope（actor / victim 经 `ftr_court_strife_actor` / `ftr_court_strife_victim` 传递，无地廷臣走 `employer` / `top_liege` 定位）；ID 沿用 `0300+` 顺排并避开已用号；选项 `name` 三段式编号且双语同步；烈度增量走 `ftr_court_strife_{low|mid|high}_intensity_value`（跨档复用的既有常量）。
...
| 0600 | 有党羽者（**已入营党羽**，含拥护君主 / 共治者者；可为无地廷臣） | 文明族 `ftr_court_impeach_pulse_effect` | 弹劾内阁・发起：具疏弹劾（→0601）／按下 |
| 0601 | 最高领主 | 0600.a（letter_event） | 弹劾裁决：准奏开除内阁 / 留中 / 亲自调停（外交≥高技能，降温）/ 含糊拖延；**站边均耗威望** |
| 0610 | **已入营党羽** A | 文明族 `ftr_court_debate_pulse_effect`（topic=1）/ `ftr_court_claim_debate_pulse_effect`（topic=2） | 朝堂论战・发起：当廷发难（→0611）／隐忍。对手 B 须与 A **不同阵营**（同池再抽，`same_camp` 排除） |
...
| 0630 | **已入营党羽** A | 文明族 `ftr_court_appraisal_pulse_effect`（仅行政类政体；否则回落 M2） | 月旦评・发起：褒而扬之（→0631）／清议贬之（→0631）／罢手。被评者须与 A **不同阵营** |
...
| **条目级可执行性过滤** | 档内各族条目的 `trigger`（2026-09 新增） | 互殴族需 `ftr_court_has_two_participants_trigger`（参与者 ≥2）；文明族需 `ftr_court_realm_has_camp_backer_trigger`（朝中 ≥ 1 名已入营党羽）。不满足者**剔出抽签池、其余条目权重重新归一**，杜绝「抽中一族却选不出人」的概率损耗（§4.3） |
...
| 关键人物观望抑制 | `ftr_court_wait_key_figure_ai_modifier` | 0200.f 观望 `base` 35 → **45**；**关键人物**（`ftr_court_wait_key_figure_trigger`：① 军力 ≥ 领主 `ftr_court_wait_key_vassal_strength_ratio_value`=10% 的封臣 ② 内阁成员 ③ 君主宫廷职位持有者 ④ 君主配偶 ⑤ 君主继承人 ⑥ 强力封臣）再 **−35**——举足轻重者不得轻易置身事外；**性格抑制**（**不作完全豁免**）：强档 `content` / `craven` / `humble` **+25**（净 −10）、常规档 `paranoid` / `patient` / `shy` / `lazy` / `trusting` **+18**（净 −17），两档互斥 |
...
| 分支 | 条件 | 事件 | 结果 |
|---|---|---|---|
| ① 上疏请立储 | 君主**尚无储位**（`NOT = { ftr_court_realm_has_crown_prince_trigger }`），且存在党羽规模达标的合格参与者 | `0520`（root = 君主） | 允准 → `ftr_court_establish_champion_coregency_effect` 统一落地（继承人立储 + 视条件建共治；非继承人取得官职 / 摄政型共治）；拒绝 → 党羽大幅交恶 + 烈度上升（04 §2.3） |
...
落地后统一 `set_diarch = CHAMPION` + `set_diarchy_swing = 40`——**被逼立共治者的初始天平一律 40**（共治型与官职 / 摄政型统一，内联于统一关节，见 04 §2.1），不再按类型分流。
