...
- **`vassal_stance` 只能作 `every_vassal`（同类迭代器）的直连过滤器** —— 放进 `limit` 会报 `Unknown trigger`（见 [03](03-触发器与效果.md) §13.1）。
- **`block_firing_councillor_effect` 是引擎级锁定** —— 被锁定者无法被解雇，到期自动失效；调用时给定年限即可实现"受保护阁职"。
- **内阁职位空 trigger 的两种相反含义** —— `can_fire` / `can_reassign` 空块 = **yes**；`auto_fill` / `inherit` / `can_change_once` 空块 = **no**（见 §1 与 [16](16-多系统选型指南与开发实践.md) §5.1）。
