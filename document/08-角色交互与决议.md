...
- **决议 `cost` 块由引擎负责** —— 资源写在 `cost = { influence = … / prestige = … }`，引擎自动收取、渲染消耗提示，并在**资源不足时自动灰显**该决议。因此效果内**不要再重复扣除**，`is_valid` 也不需再重复写资源门槛。
- **裁决类决议要补 `<key>_confirm` 本地化键**（确认按钮文案），否则界面显示裸键。
- **交互 AI 块的 `base` 不接受 script value**（报 `Malformed token`）—— 脚本值走 `modifier = { add = … }`（详见 [04](04-修饰符与数值计算.md) §8.1）。
- **`cost` 里不能用「第二步选项」scope** —— 两步决策（`widget` + `item = { value = X }`）里的 `scope:X` **只在第二步存在**；而 `cost` 在首步与 AI 评估时就会被求值，引用它会报 `Undefined event target 'X'`，并按被评估角色数**刷屏**（实测一轮 3 万条）。要按选项差异化定价，改用**随时可求值的等价条件**（如按君主 / 领主的政体判定），或把差额放进 `effect`（第二步，scope 可用）。
