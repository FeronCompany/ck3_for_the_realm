...
### 7.1 带条件的提示：`custom_tooltip` / `custom_description` 的域规则

这两个 Effect 会出现在**两个不同的域**里，写法**完全不同**，混用是最常见的静默失败点：

```paradox
# ── 效果域（effect / immediate / option 主体 / on_accept …）——
#    只接受 effects，不得直接塞 trigger 子键；
#    要让提示"按条件显示"，必须外套 if = { limit = { … } }。
if = {
	limit = { government_allows = administrative }
	custom_tooltip = { text = ftr_my_admin_only_tt }
}

# ── 判定域（is_valid / is_shown / limit / send_option.is_valid …）——
#    直接裸写触发器，原版统一写法。
custom_tooltip = {
	text = ftr_my_requirement_tt
	has_realm_law = single_heir_succession_law
}
```

> **实测**：原版 `common/character_interactions`、`common/decisions`、`common/scripted_effects`、`events` 中，
> 效果域的 `custom_tooltip` 内出现 `trigger` 子键的次数为 **0**。
> 写上并不会报错——**提示只是不按条件显示**，因此极易漏检。
> `custom_description` 同理：效果域里它只描述"将要发生什么"，判定域里才能带裸触发器。

**调试用途**：把判定条件直接写进判定域的 `custom_description`，可在 UI 上看到"这一条为什么不满足"，
比逐条注释 trigger 快得多（见 [16](16-多系统选型指南与开发实践.md) §10.1）。
...
| 约束 | 症状 |
|---|---|
| `vassal_stance` 只能作 `every_vassal`（同类迭代器）的**直连过滤器** | 放进 `limit` 报 `Unknown trigger` |
| **触发器语境**没有 `every_character_war`（仅效果语境有） | 「全部战争」须写成 `NOT = { any_character_war = { NOT = { … } } }` |
| 列表迭代器在**触发器语境**不能包 `limit` | 报 `Unknown trigger: limit`（`limit` 属效果侧语义） |
| 在 `var:X ?= { … }` 里做**变量操作**（`is_target_in_variable_list`、`add_to_variable_list` …） | `?=` 得到的是 **weak scope**，报 `This scope doesn't support variables`。**已用 `has_variable` 守卫时改用强引用 `var:X = { … }`**（原版通用写法，如 `var:stop_host = { … }`）；否则先 `?= { save_scope_as = tmp }` 固化，再在 `scope:tmp` 上操作 |

### 13.2 列表清空与遍历安全

```paradox
# 逐个排干名单：while + count 上限（见 §4.1）
while = {
	count = 100
	limit = { any_in_list = { variable = my_list  always = yes } }
	ordered_in_list = {
		variable = my_list
		order_by = <评分>
		save_scope_as = tmp
	...
	scope:tmp = { <处理> }
	remove_from_list = { target = scope:tmp  list = my_list }
}
```

> **铁律**：遍历过程中**不要修改被遍历的列表本身**（会让迭代器错位或提前终止）。要边处理边删，先把成员取进临时 scope，处理完再删。

### 13.3 权重与随机

- `random_list` 的**权重槽只接受数值字面量**，不接受 script value。用脚本值当权重时，引擎在**加载期 PostValidate 直接失败**（`PostValidate of effect 'random_list' returned false`），运行期报 `Random list with no options, or negative scores`，该列表**实际不生效**（整个 50/50 分支静默失效）。
  - 需要"按动态概率二选一"时，改用 `random = { chance = <script value> … }` + `if` 互补 guard：
    ```paradox
    remove_character_flag = my_won_flag        # 掷骰前先清标记
    random = { chance = my_win_chance_value  <win effects，内含 add_character_flag = my_won_flag> }
    if = { limit = { NOT = { has_character_flag = my_won_flag } }  <lose effects> }
    ```
    （`chance` 槽接受脚本值；`random` 没有 `else`，故用标记反查落空分支。）
  - 需要按档位给不同权重时，把分档权重直接写在表里、用互斥 `trigger` 区分档位。
- `random` / `random_list` 的 `modifier` 是**先 `add`、后 `factor`**（见 §5.2）。

### 13.4 静默施加与提示控制

- 不想让效果刷 tooltip 时，用 `hidden_effect = { … }` 包住整块（如直接 `change_diarchy_swing`，绕开带 tooltip 的原版封装）。
- 效果域 `custom_tooltip` / `custom_description` 的域规则见 §7.1。

### 13.5 `root` 不等于「当前域」

被复用的 `scripted_effect` / `scripted_trigger` **不要假设 `root` 就是当前域**：`root` 始终指向这条脚本链的**起点**。同一个 effect 若既被事件调用（root = 角色），又被 `on_combat_end_*` 之类的 on_action 调用（**root = 战斗方 `combat_side`**），effect 内的 `root` 就会变成战斗方：

| 写法 | 报错 |
|---|---|
| `this = root` | `Left side and right side during comparison were of different types (left was 'character', right was 'combat_side')` |
| `target = root`（好感 / 关系） | `add_opinion effect [ target character was null ]` |

**写法**：进入通用 effect 后立刻固化当前域，之后一律走 scope：

```paradox
ftr_my_shared_effect = {
	save_scope_as = ftr_my_subject          # 当前域 = 被处理角色（而不是 root）
	...
	scope:ftr_my_holder = {
		add_opinion = { target = scope:ftr_my_subject  modifier = my_opinion }
	}
}
```

只有能确定"调用方 root 就是目标角色"时（事件本体、`on_title_gain` 等），才可以直接写 `root`。
