...
6. **⚠ 复制本类型做「太子」时必须裁剪 swing0 档参数**：`liege_may_voluntarily_cede_authority` 的**全游戏唯一作用**是给君主的「摆动天平」交互解锁「自愿让权」选项（选中即给共治者 **+20 / +30 / +40** swing，并跳过「天平已到底」校验、AI 君主选择权重最高）——储君位不需要它，误留会使「君主摆动天平」反助共治者；`diarchy_transition_into_co_emperorship_on_majority` / `half_sop_swing_transfer_over_on_majority` 是幼帝成年转正专用，太子位同样不应携带。mod 的 `ftr_diarchy_crown_prince` 已按此裁剪（详见 03 §3.1）。
...
判断"是不是行政类政体"只有一种正确写法：

```paradox
government_allows = administrative                     # ✅ 覆盖全部行政型
government_has_flag = government_is_administrative     # ❌ 仅 administrative_government 独有
```

`government_allows = administrative` 等价于 `government_rules: administrative = yes`；而 `government_is_administrative` 只挂在 `administrative_government` 本体上，用它判断会把**天朝制 / 日本行政府 / 官僚制 / 草原行政**全部漏掉（它们只是 `administrative = yes`）。该 flag 仅限**专指行政制本体**的场合（如转政体决议 `become_admin_decision` 的选项）。
