...
选族（on_action ftr_court_struggle_main_pulse：求值域落到 story，random_list **枚举全部入口**；
      每行一个族、自带 trigger 做排除——**本层不再按烈度分区**，新增入口只需加一行）：
  权重 50   互殴手段族（选 actor/victim → 0300–0320 / 0410 / 0420 / 0430）  trigger：烈度 ≥90
  权重 50   强出口族（0003/0004 通知 → 0200 / 0140 / 0150）               trigger：烈度 ≥90
  权重 50   党羽逼宫（0520 请立共治 / 0530 逼宫退位，见 §13）              trigger：烈度 ≥90
  权重 50   党羽请立共治（同族 effect，§13 情形一）                        trigger：烈度 <90 且尚无共治
  权重 50   无地拉锯（0500；effect 内再要求天平 ≥50 且有共治者）           trigger：烈度 ≥90
  权重 50   共治者游说反扑入口（**任意天平值**；effect 内要求已有共治者 + 冷却允许）  trigger：烈度 ≥90
  权重 50   文明争斗族（M1–M6；高烈度下与其他族等权并列）                   trigger：烈度 ≥90
  权重 100  文明争斗族（低烈度下本层唯一常驻入口，与空过等权）              trigger：烈度 <90
  权重 100  **无事发生（空过）**：保留 100 权重的"朝堂风平浪静"概率，无任何游戏内提示；
            调大 = 更平静，调小 = 事端更密
            ⇒ 高烈度池：6 族各 12.5% · 空过 25%
            ⇒ 低烈度池：有共治 = 文明 50% · 空过 50%；无共治 = 文明 40% · 请立 20% · 空过 40%
```
...
拉锯**完全由事件族承担**。**`0500` 暗中策动**：天平 ≥50 且存在共治者时，主时钟随机派发无地参与者（每轮 25%），成功（**85%**）push swing −5 向君主，失败则风声走漏、声望受损并与君主交恶；另可选花 60 金币稳妥收买共治者左右。**共治者反扑**：`0510` 事件已于 2026-09 **被「党羽游说对垒」取代**（`ftr_diarchy_backer_lobby_pk_effect`）——**任意 swing 值**均可发动，前置仅「共治者党羽不落后于君主」（`ftr_diarchy_backer_lead_value >= 0`）；双方各抽一名党羽按「智谋 + 外交」对垒，胜方把天平推动 ±5，并以事件 `ftr_diarchy.0800` / `0801`（**仅一个确认选项**）通报主君、共治者与两名说客。
...
| 0510 | 共治者 | **已停用派发**（2026-09 由「党羽游说对垒」取代） | 原共治者反击：天平 ≤30 且党羽不落后时 +5；现由 `ftr_diarchy_backer_lobby_pk_effect` 承接，**任意天平可触发**，结算后发 0800 / 0801 通报事件（§9.6、03 §7.2） |
...
| `common/on_action/ftr_court_on_actions.txt` | 准入创建、失效收场 + `ftr_court_struggle_main_pulse`（**枚举全部入口**：random_list 每行一族、条件写在该行 trigger 里，本层不再按烈度分区；**权重调平**＝各族一律 50、低烈度文明族 100、另设 **100 权重的「无事发生」空过项**） |
