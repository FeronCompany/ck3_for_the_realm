...
- 效果 `ftr_diarchy_quarterly_swing_drift_effect`：
  ...
  2. **档位派发拉锯事件**：天平 ≥ 70（共治者强势）→ 25% 概率派 `ftr_court_struggle.0500` 给一名**非共治者党羽**的有地封臣；天平 ≤ 30（君主强势）→ 25% 概率派 `ftr_court_struggle.0510` 给共治者本人。
...
**④ 主君反制**：§4.4-A 的"主君可降 swing / 拉拢封臣"不依赖主动交互——拉锯由事件族承担：`0500`（≥70 时主时钟派发，push swing 向君主）+ 君主侧的背书（01 事件 0200 的君主选项）经季度背书差回拉，无需新代码。
...
| `events/ftr_diarchy_events.txt` | 0100 警告 / 0220 表态 / 0002 阶段 A 结算（hidden）/ 0210 超时兜底（hidden）/ 0600 发落废君 / 0200 授土提醒 |
