...
| 阶段 | 位置 | 说明 |
|---|---|---|
| ① 临时独立公国 | `ftr_court_prepare_rebellion_effect`（0110.b / 0120.a / 0140.a / 0160.c） | **仅无地者**：`ftr_court_create_temp_duchy_effect` 建公国 → `create_title_and_vassal_change(type = independency)` + `becomes_independent`，**脱离对君主的臣属**（独立头衔） |
| ② 精锐部队 | 同上 → `ftr_court_spawn_rebel_troops_effect` | 重装步兵 / 重装骑兵各 `ftr_court_rebel_maa_stacks_value(3)` 队 ＋ 攻城器械 `ftr_court_rebel_siege_stacks_value(1)` 队（投石机 / 射石机 / 抛石机，按已发现革新）；出兵地 = 临时公国首都，兜底君主首都 |
...
| 文件 | 内容 |
|---|---|
| `common/story_cycles/ftr_court_struggle.txt` | 容器、可视化、effect_group A/B/C、生命周期（主时钟 = 心跳 + on_action 选族；参与询问刷新带「未询问候选」短路）、**临界政变征询季度 effect_group**（0160，`months = ftr_court_coup_ask_interval_months_value`）、**幼主成年补立太子共治检测**（每拍调 03 的 `ftr_diarchy_promote_adult_heir_effect` → 事件 `ftr_diarchy.0700`，03 §2.2 1.3） |
