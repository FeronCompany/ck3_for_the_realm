...
>
> 关联文档：`01-朝堂权力角逐-系统设计.md`（同仓同目录）。本文与 01 接线：01 内支持者的回报统一走本系统的**论功行赏**，接线点见 §5。
>
...
- 功勋是朝堂角逐中支持者的回报货币：支持者凭实打实的「拥立功」，在其押注对象登基后兑奖。
...
**隐藏运算影子（仅引擎内部）**：引擎不能按修饰符读取好感净额（API 限制见 [04](../04-修饰符与数值计算.md) §8.1）。为使"摘旧挂新"能算出新值，每名效劳方身上保留四条影子（**不算记录、不展示、不参与任何业务语义**）：
...
| `ftr_personal_merit_backing_deduct_effect`（参数 `AMOUNT`） | 「被劝背书」回调：从默认 +10 中扣回差额（= 主动 − 被劝） |
| `ftr_personal_merit_redeemable_value`（script value，参数 `HOLDER`） | 某记功方视角下对效劳方的可兑净额 |
| `ftr_personal_merit_battle_credit_effect` / `ftr_personal_merit_has_credit_trigger` / `ftr_personal_merit_backing_eligible_trigger` | 战斗计入记账 / 是否可赏 / 背书对象是否可产功勋 |
...
> 实现要点：`root` 为战斗方（见 [06](../06-事件系统与on_action.md) §12）；每条侧含多个 army（各有 owner）与多个参战者——遍历时应以**军为单位**定位 owner / 判定战争主攻主防，避免把"同侧盟军私兵"或"同侧他国援军"误记。
...
| 行为 | 触发点（01 既有入口） | 功勋 |
|---|---|---|
| **主动公开背书**某合格权贵 C | `ftr_court_apply_backing_effect` 公开分支 | 拥立账(C) **+10** |
| **主动秘密背书**某合格权贵 C | `ftr_court_apply_backing_effect` 秘密分支 | 拥立账(C) **+10**（密账照记，登基后兑奖时才揭晓） |
| **被动被劝而后背书** C | 游说成功 / 0200 链中"接受劝进" | 拥立账(C) **+5**（非主动发起，减半） |
| **劝人背书**成功（劝说者本人背书对象即 C） | `ftr_court_persuade_backing_interaction` 成功分支 | 拥立账(C) **+5**（并入劝说者既有拥立账；若劝说者并无背书对象，则本次不计——防止无主账漂移） |
| **主动背弃 C**（撤回 / 倒戈改投） | `withdraw_backing` on_accept、`ftr_court_apply_backing_effect` 改投分支 | 拥立账(C) **清零** + 01 背弃惩罚链（`ftr_court_betrayed_opinion` 等）；**从征账不受影响** |
| **被动放弃背书**（C 出局 / 被清算 / 死亡 / 政权瓦解） | 01 收场与清理链 | 拥立账(C) 清零（被动结清），**无任何惩罚** |

> 支持现任君主（背书目标为君主本人）不产功勋：该行为走 01 既有"表忠 / 拥戴君主"回馈（backer_opinion / 忠诚好感 / 天平影响），不入功勋账。
...
sequenceDiagram
    ...
    V->>C: 表态支持（决议 + 0200 → apply_backing）
...
无地廷臣不能主动发起交互；其背书只能经**事件等价入口**：0200（择主站队，主动表态）、被游说成功、或年度论功事件前后缀。凡由事件促成且为**效劳方主动决断**者按「主动背书 +10」计；凡**经劝说后才同意**按「+5」计；二者不叠加（同一次站队只取其一）。
...
- 如需再扩充（如献城、救驾），按 §5.1 的接线点追加即可，本文不预埋。
...
| 接入口 | 处理 |
|---|---|
| 原版 `grant_titles_interaction` / `grant_governorship_interaction`（行政授总督） | 无需 override：统一挂原版 `on_title_gain` 回调（`ftr_personal_merit_title_gain_on_action` → `ftr_personal_merit_consume_on_title_gain_effect`），以「新 holder == 授地者」过滤后按头衔档位扣功 |
| 本 mod 事件直接授土（含 §4.4 年度授伯爵领） | 事件授土同样触发 `on_title_gain`，钩子自动扣减；需要即时扣减时可显式调用 `ftr_personal_merit_consume_on_grant_effect` |
...
| 项 | 取值 |
|---|---|
| 落地机制 | 授职时调用原版 `block_firing_councillor_effect`（引擎级锁定，年限 = `ftr_personal_merit_council_tenure_years_value` = 5） |
| 计时标记 | 受勋者另打 `ftr_personal_merit_council_tenure` flag（5 年自动清除，仅作计时 / 排查用，判定不依赖它） |
| 失效 | 届期内领主（含 AI）无法将其换下；受勋者死亡 / 丧失职位资格（被监禁、叛国定罪等）时席位正常轮换 |
...
| 01 既有入口 | 改动 |
|---|---|
| 背书落地 `ftr_court_apply_backing_effect`（公开 / 秘密） | 若目标合格（§3.2）→ `ftr_personal_merit_backing_add_effect(C, +10)` |
| `withdraw_backing` on_accept | 主动结清拥立影子（清零）；沿用背弃惩罚 |
| `ftr_court_apply_backing_effect` 改投分支 | 先清旧主拥立账（清零）再记新主（如合格则 +10） |
| 游说成功 / 0200 择主（劝进分支） | 按 §3.2 / §3.3 记 +5 / +10 |
| 现任最高领主的战斗结算 | 挂 §3.1 的 `on_combat_end_*` 回调 |
| 年度论功 | `random_yearly_everyone_pulse` 10% 钩子 + 事件族（§4.4） |

> 说明：支持者回报由登基后任意时点的**功勋兑现**（§4 全部入口）承担，君主可分次、分批论功，无需赶在继位结算期统一处理（规避继承时序风险）。
...
| 支持者表态（产功勋侧） | 沿用 01 表态支持（决议 + 0200）既有 AI；新增因子：功勋可兑且本人未来收益预期高（对方登基概率高）时更愿**公开背书**。 |
...
| ID / 键 | 类型 | 说明 |
|---|---|---|
| `ftr_personal_merit.0001` | 年度论功事件（root = 现任最高领主） | §4.4 四选项 |
| `ftr_personal_merit_reward_gold_interaction` | 角色交互（领主→臣属） | 主动赏钱，两档 |
| `ftr_personal_merit_request_gold_interaction` | 角色交互（臣属→领主） | 请赏钱 |
| `ftr_personal_merit_request_council_interaction` | 角色交互（臣属→领主） | 请以内阁职位抵赏（5 年保护） |
| `ftr_personal_merit_annual_on_action` | on_action（`random_yearly_everyone_pulse`） | 10% 触发 + 选人 + 调度 |
| 授地抵扣 | 挂原版 `on_title_gain`（`ftr_personal_merit_title_gain_on_action`） | §4.2 挂接 |
| 内阁保护 | 撤换入口过滤 | §4.6 |

> 交互的本地化键：`<key>`（交互名）+ `_desc` + `_text`（由 `notification_text` 引用的弹窗正文）双语成对，新增文本前加 `<FTR>`；事件 title/desc/option 双语成对。数值文本走 custom tooltip 引用 script value，不硬编码进文案。
...
| `ftr_personal_merit_backing_active_premium_value` | 5 | 主动背书 − 被劝背书的差额（`backing_deduct_effect` 回调用） |
| `ftr_personal_merit_council_tenure_years_value` | 5 | 内阁职位保护年限（flag 计时） |
...
| 文件 | 内容 |
|---|---|
| `common/opinion_modifiers/ftr_opinion_modifiers.txt` | 意见修饰符 `ftr_personal_merit_opinion` |
| `common/script_values/ftr_personal_merit_values.txt` | §8 全部常量 + 计数 / 可兑 script value |
| `common/scripted_effects/ftr_personal_merit_effects.txt` | 影子记账封装（含关系校验前导）/ 同步 / 消耗 / 授地抵扣 / 战斗记功 / UI 构建 |
| `common/scripted_triggers/ftr_personal_merit_triggers.txt` | 资格 / 可兑 / 计入判定 |
| `common/character_interactions/ftr_personal_merit_interactions.txt` | 3 条交互（主动赏 / 请赏 / 请职） |
| `events/ftr_personal_merit_events.txt` | `.0001` 年度论功 |
| `common/on_action/ftr_personal_merit_on_actions.txt` | 战斗结算 + 年度脉冲 + 授地抵扣（追加语义） |
| `common/scripted_guis/` + `gui/ftr_personal_merit_window.gui` | 功勋面板窗口与快捷奖赏 |
| 双语本地化（`ftr_personal_merit_l_*.yml`） | 交互 / 事件 / 窗口文案（english / simp_chinese 成对） |

**实施期登记项**：
1. 新增的 on_action 挂载点须登记进校验脚本白名单 `VANILLA_ON_ACTION_HOOKS`（本系统需加 `on_combat_end_winner`、`on_combat_end_loser`、`on_title_gain`；口径见 AGENTS §5.5）；
2. 授地抵扣走 `on_title_gain` 追加语义，**不覆盖**原版 `grant_titles_interaction` / `grant_governorship_interaction`。
...
10. `error.log` 无本系统相关报错；双语本地化无孤儿键。
