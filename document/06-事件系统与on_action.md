...
| 约束 | 说明 |
|---|---|
| **事件 ID 只能为纯数字** | `00A1` 这类含字母的 ID 会被判为非法；分带编号一律用 `0000`–`9999` |
| **延迟事件到期会"二次校验" `trigger`** | 派发时成立、到期时可能已不成立。需要"数据还在才触发"时，把校验写进事件自身 `trigger`（变量存在 + `var:X ?= { is_alive = yes }`），失效则静默丢弃 |
| `on_combat_end_winner` / `on_combat_end_loser` 的 `root` 是**战斗方** | 想定位参战角色须经"军 → `owner`"，并据此判定战争的主攻 / 主防方 |
| `on_title_gain` 会被**所有**授土路径触发 | 包括事件与效果内的授土，不只是原版交互；钩子内要按"新 holder == 授地者"等条件过滤 |
| `quarterly_playable_pulse` 的 `root` 是**可玩角色**，且只覆盖 playable | 无地 / 未建政等不可玩角色不会被扫到 |
| **继位结算期（`on_death` → 继承）内避免做重活** | 该时段上下文不稳定（持有者、头衔、共治都可能正在变），易出现"读了半途状态"；需要结算的数据用**惰性结清**——在下一次读写时再补算，不依赖继位钩子 |
