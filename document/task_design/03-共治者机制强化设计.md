...
| 无地限制 | 沿用 01 约束：**无地者不能主动发起决议与交互**；无地共治者的土地诉求改由**事件路径**承载（§4.0 每 3 年提醒；**仅限名下无任何头衔者**——持有无地虚衔者授地会被收回） |
...
- **共治锁定（2026-09 保留）**：`is_valid_showing_failures_only` 以 `custom_tooltip = { text = ftr_diarchy_designate_heir_no_diarchy_tt  NOT = { ftr_diarchy_is_heir_co_rule_type_trigger = yes } }` **判断是否存在「共治型」共治者（含太子共治 / 共治皇帝 / 共治君主 / 幼年君主）**——存在则**不得立储 / 改储**（改储须先走 §3.2 废太子或结束共治）。**官职线共治（摄政 / 宰相府 / 大秘书处）不拦截**：立储照常，只写继位、不建太子共治（建储侧 `ftr_diarchy_can_start_crown_prince_trigger` 仍要求当前无任何共治）。`ftr_diarchy_abolish_crown_prince_decision` 只对 `ftr_diarchy_crown_prince` 可见。
- 写入沿用引擎效果 `set_designated_heir`；**不支持清空**，只支持再指定他人覆盖。

**1.2 立储只写继位，太子共治改由异步询问建立（2026-09 重构）**：指定继承人动作**只调用 `set_designated_heir`**，**不再即时建立**太子共治（原 `on_accept` 内的 `try_start_diarchy` + `set_diarch` 已移除）。是否建立改由**异步通道**决定：
- **通道（2026-09 去重：单路）**：**唯一触发源** = on_action `ftr_diarchy_crown_prince_ask_on_action`（挂 `random_yearly_everyone_pulse`，年度脉冲），调用统一效果 `ftr_diarchy_promote_adult_heir_effect`（root = 君主）。**原 01 故事主时钟的同类 `triggered_effect` 已移除**——曾造成同一询问被两处重复派发。效果自带 30 天冷却 flag 与「已拒」变量，进一步防重复。
- **触发条件**：`ftr_diarchy_can_start_crown_prince_trigger`（行政类政体 + 独立 + **主头衔 ≥ 王国** + **当前无任何共治**）且指定继承人**成年可用**、**尚未就同一继承人被拒过**、且不在冷却内 → 触发询问事件 **`ftr_diarchy.0710`**（root = **君主**）。
- **事件结果**：**同意** → 复用 `ftr_diarchy_try_start_crown_prince_effect` 建立太子共治（初值 = 主动立储基线 **10**，`ftr_diarchy_crown_prince_voluntary_initial_swing_value`），并触发 `ftr_diarchy.0700` 回执新太子；**拒绝** → 被指定继承人对君主 **−30** 好感（`ftr_diarchy_corule_refused_opinion`），写变量 `ftr_diarchy_corule_ask_declined`（**同一继承人不再重问**），并调用 `ftr_diarchy_release_appointment_block_effect` **解除该继承人的任命封锁**（允许其重新出仕）。
- **重问口径**：`designate_heir_interaction.on_accept` 会清除 `ftr_diarchy_corule_ask_declined` ⇒ **改储后对新继承人重新询问**；接纳后共治已存在 → `ftr_diarchy_can_start_crown_prince_trigger` 不成立，自然不再询问。
- **立储当下即执行「留守宫廷」约束（2026-09 顺序修复）**：`on_accept` **先**对受爵者调用 `ftr_diarchy_enforce_crown_prince_unlanded_effect`（辞官 + 封锁后续任命；前置 `ftr_diarchy_realm_eligible_trigger` + `top_liege = 君主`），**再**写入储位（`set_designated_heir`），并在写入后**再确认一次**储位——避免「先写储位、后把受爵者移入宫廷 / 剥夺头衔」导致引擎重算继承把刚写入的指定清掉（实测 BUG：人来了宫廷但储位没生效，且仍可在 `is_shown` 中被再次选中）。同时（旧储 ≠ 新储时）解除**旧储**的任命封锁（见 §3.1「太子留守宫廷」）。
- **引擎硬约束**：`ftr_diarchy_try_start_crown_prince_effect` 内 `try_start_diarchy` 与 `set_diarch` 仍**连写、不得加 `has_active_diarchy` 守卫**（原版起手式，见 §9 维护注记）。

**1.3 未成年指定继承人：不即时询问，成年后由同一异步通道询问**（口径）：
- 未成年继承人不满足共治者资格（`is_diarch_able` 经 `regency_for_personal_reasons_trigger` 判定；未成年仅在 `diarchy_type_is_junior_emperorship` 类型下豁免，太子类型不携带该参数）⇒ `ftr_diarchy_promote_adult_heir_effect` 的 `is_adult` 守卫使其**暂不询问**；
- 待其**成年**后，上述异步通道（年度 on_action）自然触发 `ftr_diarchy.0710` 询问君主，同意即建共治并触发 `ftr_diarchy.0700`（告知君主 + 消息回执新太子）；
- 询问同样受「当前无共治」约束：**已有任何共治时不询问**，须先结束共治——这是**太子共治**的准入口径（§2.2 1.1/1.2），储位本身不受影响（指定继承人照常生效，只是暂不形成共治）；
- 共治请愿（01 `0220`）路径：授权对象 = **其背书对象**（若有，且非君主本人），否则请愿者本人；若该对象未成年则不即时共治（`ftr_court_champion_eligible_diarch_trigger` 拦截）→ 待其成年同样由上述通道处理（**请愿路径不再单独偏袒 `designated_heir`**）；
- **交互提示**：`designate_heir_interaction.on_accept` 改为**单条**提示 `ftr_diarchy_heir_corule_async_tt`（以 `ftr_diarchy_realm_eligible_trigger` 为条件：「是否册立太子共治由宫廷日后再议」），替换原「成年即建 / 未成年补立」两条即时提示。写法为**效果域惯用式** `if = { limit = { … } custom_tooltip = { text = … } }`（效果域的 `custom_tooltip` 不接受 `trigger` 子键，见 [03](../03-触发器与效果.md) §7.1）。
...
flowchart TD
    ...
    C --> E[选定继承人]
    D --> E
    E --> F[仅写 set_designated_heir<br/>不即时建共治 1.2]
    F --> G{异步通道：01 主时钟 / 年度 on_action<br/>行政类政体 + 独立 + ≥王国 + 无共治<br/>且继承人成年可用? 1.2}
    G -- 否 --> H[暂不询问<br/>改储则重新询问]
    G -- 是 --> I[事件 ftr_diarchy.0710 询问君主]
    I -- 同意 --> J[建立太子共治<br/>try_start_diarchy + set_diarch]
    I -- 拒绝 --> K[被指定继承人 −30 好感<br/>同一继承人不再重问]
    J --> L[触发 0700 回执新太子]
```
...
|---|---|---|---|
| 指定继承人交互重定义（`designate_heir_interaction`） | 覆盖 | `common/character_interactions/ftr_diarchy_interactions.txt` | 同名重定义原版对象 + `###### OVERRIDE ######`；1.1 扩展候选 + 1.2 分支 + 独占/锁定判定（**2026-09：只锁共治型**） |
...
- **退出（自动）**：diarchy 类型的 `end` 块为 `always = no`（**全部由 on_action 判定**，不加 `end` 块条件）。on_action `ftr_diarchy_crown_prince_invalidate_on_action`（挂 `quarterly_playable_pulse`，root = 主君）满足任一即 `end_diarchy = yes`：
  1. **政权不一致（2026-09 修复：兼容廷臣）**：共治者**既非主君封臣**（`top_liege` / `liege`）**也非主君廷臣**（`employer`）——即 `diarch = { NOR = { top_liege ?= root  liege ?= root  employer ?= root } }`。**原判据只查 `top_liege`，而廷臣没有领主链**（其归属字段是 `employer`）：太子被「留守宫廷」拉进宫成廷臣后该判定恒成立，导致**每季度脉冲误解除共治**（实测 BUG）；
  2. **级别反超**：共治者 `highest_held_title_tier ≥ 主君`（储君已然坐大）；
  3. **血缘断裂**：共治者**既非**君主的兄弟、**也非**其子女、**也非**其孙辈——即 `diarch = { NOR = { is_sibling_of = root  is_child_of = root  is_grandchild_of = root } }`；储君须为君主的**兄弟或（孙）子女**。**2026-09 修复**：原先把三条 `NOT` **平铺**进 `OR`（`NOT = { is_sibling_of = diarch }` / `NOT = { is_parent_of = diarch }` / `NOT = { is_grandparent_of = diarch }`），只要共治者是「子女」就会命中第一条 ⇒ **每季度误解除共治**（实测 BUG：共治刚建立就在 3 个月内消失，随后 `on_left_diarchy` 顺手解掉任命封锁，太子随即被外放——「自动取消 + 又去地方任职」的同一根因）。血缘判据**必须整组取反**（`NOR`）。
  同口径的**镜像守卫**写在 `ftr_diarchy_try_start_crown_prince_effect`（候选须 `top_liege = 主君` 且 `highest_held_title_tier < 主君` 才建），避免「建立 → 立刻取消」的往复。
- **太子留守宫廷（2026-09，§3.1 补）**：行政制下共治者一旦「到地方任职」（被任命为总督等 landed 头衔）便不再是主君直属封臣、共治随之解除；而原版 `admin_governor` 的 `candidate_score` **专门给「君主的共治者」加分**（`game/common/succession_appointment/admin_governor.txt:388-395`，`governor_diarch_desc`），使太子极易被外放。故对太子施加两道约束，使其始终留守主君宫廷：
  1. **解除地方任职 + 回宫**：`ftr_diarchy_enforce_crown_prince_unlanded_effect`——若目标 `is_landed = yes` 则 `force_step_down_landed_titles = yes`（**交出全部 landed 头衔**），随后**显式 `add_courtier` 归入主君宫廷**（`force_step_down_landed_titles` 只剥夺头衔、不会把角色送回宫廷——2026-09 实测修复：否则辞官后滞留在原地而非宫廷），并以 `set_location_to_default` 把所在地移回主君首都。**触发时机（2026-09 调整）：① 立储当下**（`designate_heir_interaction.on_accept`，前置 `ftr_diarchy_realm_eligible_trigger` + `top_liege = 君主`）**即时执行**——不依赖异步共治事件（0710）链路，避免事件条件不满足 / 未走到时约束不生效；**② 太子共治建立时**（`ftr_diarchy_try_start_crown_prince_effect`，与 ① 互为冗余、可安全重复）；**③ 太子一旦受衔即刻收回（2026-09 新增，关键兜底）**：on_action `ftr_diarchy_crown_prince_title_gain_on_action` 挂 `on_title_gain` / `on_title_gain_inheritance` / `on_title_gain_usurpation`（root = 新持有者）——**不等年度脉冲**，因为「不再是直属封臣 ⇒ 解除共治」可能在授衔后的**下一个 tick** 就发生，年度兜底根本来不及。闸门用 `ftr_diarchy_is_crown_prince_trigger`（要求现任领主 / 宫廷主就是太子共治所有者），故太子若因继承落入**他国**领主链、或所获头衔不在本政权，本钩子**不会**误把他塞进外国宫廷。**同批修复**：`ftr_diarchy_is_crown_prince_trigger` 原只查 `liege = { ... }`（硬 scope），太子被拉进宫成**廷臣**（无领主链、归属字段是 `employer`）后判型失败 ⇒ 留守约束与解封**全部静默失效**；现改为 `liege ?=` / `employer ?=` 两路并查。
  2. **封锁后续任命**：同一效果内 `set_appointment_timeout = { years = 150  desc = ftr_diarchy_appointment_timeout_desc }`（伪永久封锁；**原版「拒绝任命」决议同款机制**，见 `game/common/decisions/dlc_decisions/tgp/tgp_china_decisions.txt:338-383`）。
  兜底与解除：on_action `ftr_diarchy_crown_prince_keep_in_court_on_action`（挂 `random_yearly_everyone_pulse`）**每年**对在世太子重跑一次约束（覆盖「立储之后才被任命」）；**改储**时由 `designate_heir_interaction.on_accept` 对**旧储**调用 `ftr_diarchy_release_appointment_block_effect` 解封；**君主拒绝册立**（事件 0710 拒绝分支）亦解封（允许储君重新出仕）；共治结束时由 on_action `ftr_diarchy_crown_prince_release_on_left_on_action`（挂 `on_left_diarchy`，root = 君主 / `scope:old_diarch`）同样解封（`clear_appointment_timeout`），避免影响其日后受任 / 继位。
  - **重要假设**：皇位继承同为任命制（`acclamation_succession_law → appointment_type = admin_emperor`），故封锁期间太子**亦无法经「任命」被送上皇位**——本 mod 依赖 `designated_heir` 让其继位，并靠 `on_left_diarchy` 在君主驾崩 / 共治终止时立即解封。若实测发现封锁确实阻断继位，需改为「只覆盖各 governor 的 `candidate_score` 精准排除太子」方案。
...
- **01 角逐重开（2026-09 新增）**：废储后调用 `ftr_diarchy_reopen_heir_contest_effect { DEPOSED = … }`（主君域）：**第一遍**先清除君主每个合格成年子女（在世 / 成年 / 可理政 / 未在押 / 非被废者 / 性别继承法允许）的 `ftr_court_declined_contest`——**无视其此前的「明确不参与」选择**；**第二遍**按 01 的候选过滤（`ftr_court_can_be_asked_participate_relative_trigger`）逐人派 `ftr_court_struggle.0701`，由继承人自行决定是否**重回角逐**（选 a 按 0230 同一套记账重新入局并清 declined；选 b 重新打 declined）。无朝堂 story 时整段跳过。

### 3.2.1 立储与 01 角逐的接线（2026-09 新增）

- 立储落地处（`ftr_diarchy_try_start_crown_prince_effect`，**指定继承人交互**与**成年补立**两条路径的公共关节）在共治确已建立后调用 `ftr_diarchy_sync_crown_prince_contest_effect { PRINCE = … }`（主君域）：
  1. **确保太子成为竞争者**：清掉他身上残留的 `ftr_court_declined_contest`（否则 01 口径下"不许他人主动支持他"），补 `ftr_court_participating`，经 `ftr_court_add_participant_effect` 写入君主主名单并同步 story 的 `ftr_court_participant_list` + 刷新参与计数 + 抬升烈度；
  2. **其余每名竞争者**（`ftr_court_claimants`，排除太子本人）经变量传递 + 延迟 2 天收到 `ftr_court_struggle.0700`，自行决定**退出竞争**（`ftr_court_leave_contest_effect`：双名单除名、刷新计数、释放其名下党羽背书、清个体状态与临时公国，随后打 `ftr_court_declined_contest`）或**继续竞争**（新太子对其 −20 好感）。AI 退出基准 55（知足 / 怯懦 / 谦逊 / 与新太子交好 → 更倾向让位；野心 −45、第一顺位继承人 −25）。

### 3.2.2 名单一致性定期检查（2026-09 新增，01 §5.4）

- `common/story_cycles/ftr_court_struggle.txt` 的**年度** `effect_group` 内，紧随烈度维护调用 `ftr_court_clear_claimant_backings_effect`（story 域）：遍历君主 `ftr_court_claimants`，凡**仍挂 `ftr_court_backer_of` 者**（曾背书他人且从未取消）即撤销该背书——从被支持者的 `ftr_court_supporters` 名单移除自己、清站队变量、结清拥立账。
- 原因：竞争者同时充当别人的党羽会**污染 swing 的背书差**（自己既计入被支持者的党羽数、又占着"已站队"资格）。属名单维护，**不施加好感惩罚、不发通知**。
...
- **复辟政变**：被废太子**用本 mod 的 `coup` 计谋**复辟（`common/schemes/scheme_types/ftr_coup_scheme.txt` + `ftr_coup_success_effect` / `ftr_coup_failure_effect`）；成功 = 推翻自立，失败 = 囚禁或流放（由计谋机制自然触发，不做决议后的硬概率触发）。
  ...
  - **退出机制（2026-09）**：`coup` 计谋的 `agent_leave_threshold` 由 −25 压到 **−1000**，使计谋内特工不会因意愿下降被自动清退；01 的 `ftr_court_struggle.0160` 政变分支另以 `force_add_to_agent_slot`（锁 10 年）把起兵者党羽按 agent 能力强制拉入——两者共同保证共治者 / 党羽**不会中途退出阴谋**。
...
| 太子共治自动失效 | 新增 | `common/on_action/ftr_diarchy_on_actions.txt` | `ftr_diarchy_crown_prince_invalidate_on_action`（root = 主君）：政权不一致 / 共治者级别 ≥ 主君 / 血缘断裂（非兄弟・非父・非祖）→ `end_diarchy`；挂 `quarterly_playable_pulse` |
| 太子留守宫廷（2026-09） | 新增 | `common/scripted_effects/ftr_diarchy_effects.txt` + `common/on_action/ftr_diarchy_on_actions.txt` | `ftr_diarchy_enforce_crown_prince_unlanded_effect`（剥夺 landed 头衔 + `set_appointment_timeout` 封锁任命）/ `ftr_diarchy_release_appointment_block_effect`（解封）；立储时即时执行一次 + **受衔即时收回（`on_title_gain` 三路）** + 年度 on_action 兜底 + `on_left_diarchy` 解封 |
| 建储镜像守卫 | 修改 | `common/scripted_effects/ftr_diarchy_effects.txt` | `ftr_diarchy_try_start_crown_prince_effect` 增「同政权 + 级别低于主君」判定，与自动失效条件互锁，防往复 |
...
- **方案**：无地共治者**每 3 年**收到一次授土提醒事件（事件路径，符合 01 无地者约束）——**但太子除外**（`ftr_diarchy_landless_reminder_trigger` 已增 `NOT = { ftr_diarchy_is_crown_prince_trigger = yes }`：太子须留守宫廷，授土会把他变成 landed 封臣、进而解除共治，见 §3.1「太子留守宫廷」），**三选一**：
  1. **索取**——无条件获取土地；
  2. **暂不考虑**——本次放弃，后续仍会再刷；
...
- **对象口径（2026-09）**：只对**名下没有任何头衔**的共治者提醒——`ftr_diarchy_landless_reminder_trigger` 除 `is_landed = no` 外，另要求 `NOT = { any_held_title = { always = yes } }`。原因：`is_landed` **判不出无地头衔**，天朝制的宰相（首相府共治）持有无地的帝国级虚衔，划地给他也会被制度收回，提醒与授土全部空转。同一口径落在三处——发送侧 `ftr_diarchy_landless_reminder_trigger`、授土效果 `ftr_diarchy_grant_land_effect`（受地者不落 scope 即安全跳过）、事件 0210 的**事件级 `trigger`**（成立条件不在选项上重复判定：主君无地可划或请地者已持有头衔时整件事静默作废，连「推托」选项都不出现）。
...
**已知缺口——太子难达篡位门槛**：太子类型 `swing_balance` 抄自 `junior_emperorship`（按年龄 0/10/20/30），引擎每月按 `DIARCHY_MONTHLY_POWER_CHANGE = 0.25` 把 swing **向该值回落**；单次抬升仅 `minor_sop_swing_diarch_gain` 量级（≈+5）。故无持续经营时太子几乎无法抵达独断档，须依赖 §7.2 的季度背书 drift 与党羽相助。**太子初始天平两档**：主动立储 = **10**（`ftr_diarchy_crown_prince_voluntary_initial_swing_value`）、被逼立储（共治请愿 / 党羽逼宫）= **40**（`ftr_court_establish_champion_coregency_effect` 内联字面量；04 §2.1 起被逼立共治者的初始天平统一为 40，不再按类型分流）。**历史坑**：该效果原本对四种类型一视同仁地 `set_diarchy_swing = 80`（取自党羽逼宫的「逼宫所得不可虚位」口径），导致请愿立太子后 swing 一次到位 80（0220 再 +5 → 85），与本节"太子须慢慢经营"的设计相互矛盾。
...
**③ 0500 泛化**：该事件的 `immediate` 末尾带作用域兜底——当**不存在朝堂角逐 story** 时，用共治关系补全 `scope:ftr_court_story_owner`（取 `liege` → `top_liege`）。由此该事件脱离 01 的 story 也能工作，成为**共治期的日常 swing 拉锯层**，补上 §4.4 只能"一次性摊牌"的空档。
...
| `common/scripted_effects/ftr_diarchy_effects.txt` | 立储接线 / **异步询问太子共治 `ftr_diarchy_promote_adult_heir_effect`（由年度 on_action `ftr_diarchy_crown_prince_ask_on_action` 单路调用）** / 授土 / 共用结局 / 废太子 / 大赦 / 阶段 A 计时与结算 / 清君侧结算 / 征调入战 |
...
**维护注记**：
1. **已确认的引擎约束（必须遵守，否则静默失败或刷 error.log）** —— 均属通用语法 / 原版引擎知识，已迁入语法文档，此处只留索引，不再复述：
   - diarchy 类型字段与硬约束（`end_interaction` 必填、两条本地化键、`try_start_diarchy` + `set_diarch` 连写、`update_diarchy_swing_*` 前置守卫、副手候选硬规则）→ [18 共治系统详解](../18-共治系统详解.md) 附二；
   - 清空储位 `clear_designated_heir`、无继承顺位序号触发器 → [10 法律与继承](../10-法律与继承.md) §E；
   - `vassal_stance` 直连过滤、触发器语境的 `every_character_war` / `limit` → [03 触发器与效果](../03-触发器与效果.md) §13.1；
   - 交互 AI 块 `base` 不收脚本值 → [04 修饰符与数值计算](../04-修饰符与数值计算.md) §8.1；
   - 事件 ID 纯数字 → [06 事件系统与 on_action](../06-事件系统与on_action.md) §12；
   - 本地化键（`_confirm` / `war_name` / diarchy 两键）→ [07 本地化](../07-history历史脚本与本地化.md) §11、[12 活动与战争](../12-活动与战争.md) 附三；
   - 行政类政体判定 `government_allows = administrative` → [13 政体、特质与共治](../13-政体、特质与共治.md) §4.1；
   - 决议 `cost` 块语义 → [08 角色交互与决议](../08-角色交互与决议.md) §3。
...
5. 无地共治者：每 3 年授土提醒可正常触发；三选一各打一遍——索取（主君非首都伯爵领划出 / 公国连下属转移）、暂不考虑（后续再刷）、忠心事主（此后不再刷）。**太子不触发**该提醒（太子须留守宫廷）。
...
18. **太子留守宫廷（2026-09）**：行政制下指定太子后——① 太子若已有 landed 头衔（如已被任命为总督）会**立即被剥夺**并回到主君宫廷；② 对太子走「任命」流程时不进入候选（`appointment_timeout_days` > 0），AI 也不会再外放他；③ 太子被废 / 共治结束时封锁**解除**（`appointment_timeout_days` = 0），可正常受任；④ **关键验证**：主君驾崩后太子仍能继位（验证 `designated_heir` 是否绕过任命封锁）——若被封锁挡住，须改用「覆盖各 governor 的 `candidate_score` 精准排除」方案（见 §3.1 该条的「重要假设」）。
