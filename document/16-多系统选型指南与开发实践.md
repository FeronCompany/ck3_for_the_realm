...
7. **`custom_description` 自证** —— 把判定条件写进判定域的提示，UI 上直接看到"这一条为什么不满足"（[03](03-触发器与效果.md) §7.1）
...
trait/law/effect/trigger、`scop:` 拼写、`change_gold`、GUI ScriptValue/未本地化文本，
以及 2026-09 排错归纳的引擎语义陷阱：`random_list` 动态权重、事件块多份 `trigger`、事件缺 `desc`、
on_action 块多份 `trigger`/`effect`、`custom_tooltip` 内写 `trigger = { }`、weak scope 变量操作、孤儿事件。
...
| 本地化报 `Data error in loc string '<key>'` | loc 表达式里**不能写 `[scope:x.GetName]`**（报 `Failed to find type 'scope:x'`）——作用域一律用 `save_scope_as` 的**裸名** `[x.GetName]`；作为 `Custom2('...', ...)` 的参数时写 `SCOPE.sC('x')`。另：事件上下文里的 `sender` 在 loc 中也取不到（报 `Failed to find type 'sender'`），书信抬头须先 `save_scope_as` 再引用（0220 / 0601 曾因此报错） |
