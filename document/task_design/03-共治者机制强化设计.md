...
- **代价（双向）**：双向**固定值**好感惩罚 + **直接推向仇敌**——复用原版三个 opinion modifier：`designated_me_regent_opinion` / `passed_me_over_for_regent_opinion` / `designated_bad_regent_opinion`；**太子党羽**（太子 `ftr_court_supporters` 支持者名单，01 双记账）一并记恨主君（`ftr_diarchy_abolish_backer_opinion_value`）；另加**主君正统性惩罚**（`ftr_diarchy_abolish_legitimacy_value = medium_legitimacy_loss`，走 `add_legitimacy_effect` 包装）。
...
| `ftr_diarchy_swing_drift_per_backer_value` / `_swing_drift_liege_backer_multiplier_value` / `_swing_drift_cap_value` / `_swing_drift_cap_min_value` / `_swing_event_chance_value` / `ftr_diarch_counter_swing_threshold_value` | 季度背书 drift 的折算 / 君主党羽折算溢价（1.5）/ 上下限（+2 ~ −3）/ 派发概率 / 0500 派发门槛（中性 50）（§7.2） |
...
- 效果 `ftr_diarchy_quarterly_swing_drift_effect`：
  1. **背书差 → swing**：`ftr_diarchy_backer_swing_drift_value` =（背书共治者人数 − 背书君主人数 × **1.5 折算溢价**）× **1 点/人**，**夹在 −3 ~ +2**（正向上限刻意低于负向），经 `hidden_effect = { change_diarchy_swing = ... }` 静默施加（不走带 tooltip 的原版封装，避免刷提示）。**溢价理由**：共治者天然更易积累党羽（大赦天下 / 请愿 / 逼宫都把党羽记在其名下），原「共治者人数 − 君主人数」的对称算法使 drift **长期单向上行（+3/季 ≈ +12/年）**——引擎侧每月只回落 0.25（≈ −3/年），故天平会「只涨不跌」；
  2. **档位派发拉锯事件**：天平 ≥ **50（中性）**（`ftr_diarch_counter_swing_threshold_value`）→ 25% 概率派 `ftr_court_struggle.0500` 给一名**非共治者党羽**的有地封臣；天平 ≤ 30（君主强势）→ 25% 概率派 `ftr_court_struggle.0510` 给共治者本人。**0500 门槛原取强权下沿 70**，使君主侧的回拉通道启动太晚，是单向上行的第二根因。
...
17. **天平拉锯对称性（2026-09 修复）**：太子共治下，君主「摆动天平」窗口**不再出现**「自愿让权」选项（参数已删）；君主一次拉回 **−20**（带 `power_at_home` 特性 + 旅途中 −30）、共治者一次 **+10**（同前条件 +20），冷却各 2 年；季度 drift 夹在 **+2 ~ −3**，且天平 ≥ 50 起由 0500 回拉、≤ 30 起由 0510 上推 —— 长期应在 20–80 区间摆动，不再单向爬满。
