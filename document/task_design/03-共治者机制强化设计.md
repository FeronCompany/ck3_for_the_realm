...
- **独占与锁定（2026-09 松绑：只锁「共治型」）**：主君侧拦截口径 = `NOT = { ftr_diarchy_is_heir_co_rule_type_trigger = yes }`——**仅共治皇帝 / 共治君主 / 太子 / 幼年君主期间不得立储 / 改储**（原版 `NOT = { has_diarchy_active_parameter = diarchy_is_co_rulership }` 的同类口径，比「任何共治都锁」更准）。**摄政 / 宰相府 / 大秘书处等官职线共治不再冻结储位**：期间可正常立储 / 改储，但因建储侧 `ftr_diarchy_can_start_crown_prince_trigger` 仍要求「当前无任何共治」，此时**只改写继位、不建立太子共治**。**候选人本人**同样只须不是「共治型」共治者——`trigger_if = { limit = { is_diarch = yes }  NOT = { liege = { ftr_diarchy_is_heir_co_rule_type_trigger = yes } } }`，官职线共治者不影响其被立为储君。以上两条均由 `designate_heir_interaction.is_valid_showing_failures_only` **前置拦截**——不满足时交互直接失败并给出原因（提示键 `ftr_diarchy_designate_heir_no_diarchy_tt`），效果侧不再重复判定。`ftr_diarchy_abolish_crown_prince_decision` 只对 `ftr_diarchy_crown_prince` 可见 ⇒ **官职线共治（如原版 `regency`）下无"废储"入口属预期**；共治型（太子等）须先结束共治（原版 `end_interaction` 或 03 的 `depose_diarch`）或走 §3.2 废太子决议。**已存在继承人线共治者（共治皇帝 / 共治国王 / 幼年皇帝 / 太子）时不可再重新指定**——改储必须先走废立（§3.2）。锁定锚点 = **已在共治位的继承人线共治者**；仅被 `designate_heir` 指定、尚未形成共治时仍允许更换；
...
- 补立同样受「当前无共治」约束：**已有任何共治（含摄政）时不补立**，须先结束共治——这是**太子共治**的准入口径（§2.2 1.1），储位本身不受影响（指定继承人照常生效，只是暂不形成共治）；
...
- **退出（自动）**：分**两侧**——
  1. **主君侧**（diarchy 类型的 `end` 块）：主君失去独立（成为附庸）、或政体不再满足（`NOT = { government_allows = administrative }`，与准入同一口径）、或等级跌破王国。**欢呼法不再导致结束**——避免与 mod 的 `ftr_ursurp_tyrant_succession_effects`（会强加欢呼法）互斥而立即解体。
  2. **共治者侧**（on_action `ftr_diarchy_crown_prince_invalidate_on_action`，挂 `random_yearly_playable_pulse`，root = 主君）：满足任一即 `end_diarchy = yes`——① **君主与共治者不在同一政权**（共治者 `top_liege` 已非主君：被转封他国 / 独立）；② **共治者级别 ≥ 君主**（`highest_held_title_tier` 反超，储君已然坐大）。同口径的**镜像守卫**写在 `ftr_diarchy_try_start_crown_prince_effect`（候选须 `top_liege = 主君` 且 `highest_held_title_tier < 主君` 才建），避免「建立 → 立刻取消」的往复。
...
| 太子共治自动失效 | 新增 | `common/on_action/ftr_diarchy_on_actions.txt` | `ftr_diarchy_crown_prince_invalidate_on_action`（root = 主君）：政权不一致 / 共治者级别 ≥ 主君 → `end_diarchy`；挂 `random_yearly_playable_pulse` |
