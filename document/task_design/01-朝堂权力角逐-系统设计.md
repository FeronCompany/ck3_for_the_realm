...
| 游说对垒（共治者反扑；2026-09 **取代事件 0510**） | +5 / −5 | 天平 ≤30 **且共治者党羽不落后于君主**时启动（每轮 **15%**，`ftr_diarch_backlash_event_chance_value`）：双方各随机抽一名党羽按「智谋 + 外交」对垒，胜方推动 ±5；政权 / 参战者各 1 年冷却 |
...
拉锯**完全由事件族承担**。**`0500` 暗中策动**：天平 ≥50 且存在共治者时，主时钟随机派发无地参与者（每轮 25%），成功（**85%**）push swing −5 向君主，失败则风声走漏、声望受损并与君主交恶；另可选花 60 金币稳妥收买共治者左右。**共治者反击**：`0510`——swing ≤30 **且共治者党羽不落后于君主**（`ftr_diarchy_backer_lead_value >= 0`）时共治者阵营反击（每轮 **15%**），密令亲信游说把天平拉回 +5。
...
| 0510 | 共治者 | 主时钟（共治反击）/ 03 季度背书 drift | 共治者反击：天平 ≤30 **且共治者党羽不落后**时向共治者侧移轴（每轮 15%，§9.6） |
