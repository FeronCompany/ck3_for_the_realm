...
- **每个 CB 必须提供 `war_name` 本地化键**，否则宣战界面显示裸键。
- **触发器语境没有 `every_character_war`**（仅效果语境可用）——CB 的 `allowed_for_character` / `valid_to_start` 里要表达「全部战争」须写成 `NOT = { any_character_war = { NOT = { … } } }`；列表迭代器在触发器语境也不能包 `limit`（详见 [03](03-触发器与效果.md) §13.1）。
- **首府男爵领不能单独易主** —— `change_title_holder*` 作用在首府男爵领上报 `Trying to change ownership of a capital barony; change the ownership of the county instead`；批量授地 / 换持有者的循环里要 `limit = { is_capital_barony = no }` 跳过（`is_capital_barony` 可在头衔域当触发器用）。
