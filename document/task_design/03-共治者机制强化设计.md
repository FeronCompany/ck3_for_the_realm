...
- **通道**：01 故事循环主时钟（`common/story_cycles/ftr_court_struggle.txt` 的 `effect_group`，60–120 天一拍）**与**新增 on_action `ftr_diarchy_crown_prince_ask_on_action`（挂 `random_yearly_everyone_pulse`）**都会**调用统一效果 `ftr_diarchy_promote_adult_heir_effect`（root = 君主）——两路互为兜底，效果自带 30 天冷却 flag 与「已拒」变量防重复派发。
...
- **立储当下即执行「留守宫廷」约束**：`on_accept` 在 `set_designated_heir` 后**立即**对受爵者调用 `ftr_diarchy_enforce_crown_prince_unlanded_effect`（辞官 + 封锁后续任命；前置 `ftr_diarchy_realm_eligible_trigger` + `top_liege = 君主`），**不等异步事件**——避免事件链路未走到时约束不生效；同时解除**旧储**的任命封锁（见 §3.1「太子留守宫廷」）。
...
- 待其**成年**后，上述异步通道（01 主时钟 / 年度 on_action）自然触发 `ftr_diarchy.0710` 询问君主，同意即建共治并触发 `ftr_diarchy.0700`（告知君主 + 消息回执新太子）；
...
- **退出（自动）**：diarchy 类型的 `end` 块为 `always = no`（**全部由 on_action 判定**，不加 `end` 块条件）。on_action `ftr_diarchy_crown_prince_invalidate_on_action`（挂 `quarterly_playable_pulse`，root = 主君）满足任一即 `end_diarchy = yes`：
  1. **政权不一致**：共治者 `top_liege` 已非主君（被转封他国 / 独立）；
  2. **级别反超**：共治者 `highest_held_title_tier ≥ 主君`（储君已然坐大）；
  3. **血缘断裂**：君主**既非**共治者的兄弟、**也非**其父 / 祖父（`NOT = { is_sibling_of = diarch }`、`NOT = { is_parent_of = diarch }`、`NOT = { is_grandparent_of = diarch }`）——储君须为君主的**兄弟或（孙）子女**。
...
| 太子留守宫廷（2026-09） | 新增 | `common/scripted_effects/ftr_diarchy_effects.txt` + `common/on_action/ftr_diarchy_on_actions.txt` | `ftr_diarchy_enforce_crown_prince_unlanded_effect`（剥夺 landed 头衔 + `set_appointment_timeout` 封锁任命）/ `ftr_diarchy_release_appointment_block_effect`（解封）；立储时即时执行一次 + 年度 on_action 兜底 + `on_left_diarchy` 解封 |
