...
| 约束 | 说明 |
|---|---|
| 清空储位只能用 `clear_designated_heir = yes` | 写 `set_designated_heir = none` 报 `target: field not set` |
| **没有「继承顺位序号」触发器** | 引擎只提供 `every_heir` 这类**集合遍历**，拿不到"第 N 顺位"；要排序自己用 `order_by` + 打分 |
| `change_title_holder type = granted` **不产生任何提示** | 授土式更换持有者不刷通知，需要提示得自己补 |
| 贵族家族头衔判定 | 用 `is_noble_family_title`；给非家族头衔显式加 `is_noble_family_title = no` 可避免被当作家族头衔处理 |
