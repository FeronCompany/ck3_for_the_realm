...
最高领主对**背书他人的党羽成员**（`ftr_court_backer_of` 指向非君主本人、且是其直属封臣）当众施压：
...
- **退出判定**：基础 10%，顺从性格加分——懦弱 +20 / 谦卑・忠信（loyal）+15 / 轻信・公正・安于现状 +10，对君主好感 ≥40 再 +15；反向——傲慢・野心勃勃 −15 / 勇敢 −10，好感 <0 再 −15；夹在 0–60（`ftr_court_discipline_exit_chance_value`）。退出即解除背书：先从党首 `ftr_court_supporters` 除名，再清 `ftr_court_backer_of` 与拥立账，并 interface_message 通报君主。
- **代价**：被敲打者无论是否屈服都记恨（`ftr_court_disciplined_opinion` −25/10 年，可叠加）；其党首亦不满君主插手（`ftr_political_opponent_opinion`）。
- **惊君处置承接**：党首是君主的子女 / 继承人时，敲打即打上 15 年警告 flag `ftr_liege_warn_not_support_heir`（原 0041 选项 a 语义）。
- **AI**（权重内联）：烈度 ≥ 酝酿档 +30；多疑 / 记仇 +20；公正 / 安于现状 −25；与目标交恶 +20。对同一目标 5 年冷却。
